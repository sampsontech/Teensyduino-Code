...
//is 0x69. When the jumper (AD0) is connected GND then the I2C address is 0x68.
...
void loop()
{
  ...
  int Temp, xRate, yRate, zRate;

  //Read the temperature fom the gyroscope.
  Temp = readT();
...
//This function is used to read the Temperature reported by the gyroscope.
//NOTE: This value is NOT in degrees. 
//Usage: int Temp = readT();
int readT(void)
{
  int data=0;
  data = itgRead(itgAddress, TEMP_OUT_H)<<8;
  data |= itgRead(itgAddress, TEMP_OUT_L);  
